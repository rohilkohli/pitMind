...
export function CircuitMap({ apiKey }: Props) {
  if (!apiKey) {
    return (
      <div className="rounded-xl border border-dashed border-pit-stroke bg-black/40 p-4 text-sm text-pit-muted">
        <p className="font-semibold text-pit-fg">Circuit map</p>
        <p className="mt-2">
...
function CircuitMapInner({ apiKey }: { apiKey: string }) {
  ...
  if (loadError) {
    return (
      <div className="rounded-xl border border-pit-stroke bg-black/40 p-4 text-sm text-pit-muted">
  ...
  if (!isLoaded) {
    return (
      <div className="rounded-xl border border-pit-stroke bg-black/40 p-6 text-sm text-pit-muted">
        Loading circuit map…
      </div>
    );
  }
